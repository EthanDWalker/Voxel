...
void BaseVulkanImage::CreateBase(const VkImageCreateInfo &image_ci,
  ...
  VK_CHECK(vmaCreateImage(VulkanContext::allocator, &image_ci, &alloc_info, &this->obj,
...
VulkanImage<ImageType::Planar>::~VulkanImage<ImageType::Planar>() { DestroyBase(); }

void VulkanImage<ImageType::Planar>::Recreate(Vec2u32 extent, VkFormat format,
                                              VkImageUsageFlags usage_flags, bool mipmapped) {
  DestroyBase();
  Create(extent, format, usage_flags, mipmapped);
}

void VulkanImage<ImageType::Planar>::Create(Vec2u32 extent, VkFormat format,
                                            VkImageUsageFlags usage_flags, bool mipmapped) {
...
void VulkanImage<ImageType::CubeMap>::Create(u32 side_length, VkFormat format,
                                             VkImageUsageFlags usage_flags, bool mipmapped) {
  const u32 mip_levels = mipmapped ? CalculateMipLevels(Vec2u32(side_length)) : 1;
  VkImageCreateInfo image_ci =
      ImageCI(format, usage_flags, Vec3u32(Vec2u32(side_length), 1), mip_levels);
